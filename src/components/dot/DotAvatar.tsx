...
export function DotAvatar({ state = 'idle', size = 'md', facing = 0, className }: DotAvatarProps) {
  const sizeClass = {
    sm: 'size-8',
    md: 'size-12',
    lg: 'size-20',
  ...
  return (
    // Outer div handles rotation — kept separate so the bob animation doesn't conflict
    <div
    ...
    >
      <div
        className={cn(
          ...
          state === 'walking' && 'animate-[walk_0.35s_ease-in-out_infinite]',
          className,
        )}
      >
        <svg
